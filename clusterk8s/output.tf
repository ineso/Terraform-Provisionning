/*************************************************************
    Store master and nodes ips into a template file for the cluster of app
 *************************************************************/
resource "template_file" "terraform_ansible" {
  ...
  vars = {
    ...
    ip_master = "ip = ${google_compute_instance.vm_instance.network_interface.0.network_ip}"
    ...
    ip_bastion = " ${google_compute_instance.vm_instance.network_interface.0.network_ip}"
    user="instz"
...
/*************************************************************
    Store master and nodes ips into a template file for the cluster of outils
 *************************************************************/

resource "template_file" "terraform_ansible_outil" {
  template = file("${path.module}/template_script.tpl")
  vars = {
    name_bastion_priv_key = "${var.BASTION_PRIV_KEY}"
...
/*************************************************************
    return the ansible inventory as terraform output of your cluster app
 *************************************************************/
...
/*************************************************************
    return the ansible inventory as terraform output of your cluster outil
 *************************************************************/
output "script_template_outil" {
	value = "${template_file.terraform_ansible_outil.rendered}"
}
